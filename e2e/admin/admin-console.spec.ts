...
test('administrator can inspect dashboard, models, errors and audit logs', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '多参考提示词优化' }).click()
  await expect(page.getByRole('heading', { name: '多参考提示词优化', level: 1 })).toBeVisible()
  await expect(page.getByRole('button', { name: /^Gemini/ })).toBeVisible()
  await expect(page.getByRole('button', { name: /^MiniMax 官方/ })).toBeVisible()
  await expect(page.getByText('优化结果')).toBeVisible()
  ...
  await expect(page.locator('.worker-status.running')).toHaveCount(5)
})
