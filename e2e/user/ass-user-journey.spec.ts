...
test('user generates an editable storyboard from ASS', async ({ page }) => {
  ...
  await expect(page.locator('.balance-pill.yinghe .balance-value')).toHaveText('287.39')
  ...
  await expect(general.getByText('未选择时，人物镜由视频模型自由生成人物')).toBeVisible()
  await expect(generate).toBeEnabled()
  await general.getByRole('button', { name: '取消' }).click()

  await page
    .locator('.script-editor .header-actions')
    .getByRole('button', { name: '随机通用分镜', exact: true })
    .click()
  const random = page.locator('.modal').filter({ hasText: '随机通用分镜' })
  await expect(random.getByText('音乐属性')).toBeVisible()
  await expect(random.getByText('生成规模')).toBeVisible()
  await expect(random.getByText('空镜数量')).toBeVisible()
  await expect(random.getByText('人物镜数量')).toBeVisible()
