...
test('administrator can inspect dashboard, models, errors and audit logs',async({page})=>{
  await page.goto('/login');await page.getByLabel('用户名').fill(username);await page.getByLabel('密码').fill(password)
  const login=page.waitForResponse(r=>r.url().includes('/api/auth/login')&&r.request().method()==='POST');await page.getByRole('button',{name:'登录'}).click();expect((await login).ok()).toBeTruthy();await expect(page).toHaveURL(/\/projects/)
  ...
  await page.getByRole('button',{name:'模型管理'}).click();await expect(page.locator('tbody').getByText('gpt-image-2',{exact:true}).first()).toBeVisible();await expect(page.locator('tbody').getByText('doubao-seedance-2.0',{exact:true}).first()).toBeVisible()
