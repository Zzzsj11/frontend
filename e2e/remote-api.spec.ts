...
const baseURL = process.env.REMOTE_API_BASE_URL || process.env.PLAYWRIGHT_BASE_URL || 'http://124.222.219.76:5173'
...
const runId = process.env.REMOTE_E2E_RUN_ID || `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
...
async function json<T = any>(response: APIResponse, expected: number | number[] = 200): Promise<T> {
  ...
  expect(allowed, `${response.url()} => ${response.status()} ${await response.text()}`).toContain(response.status())
  return response.json() as Promise<T>
}

async function login(username: string, password: string): Promise<{ api: APIRequestContext; token: string; user: any }> {
  ...
  const api = await playwrightRequest.newContext({ baseURL, storageState: storage, extraHTTPHeaders: { Authorization: `Bearer ${token}` } })
...
test('remote API contract, authorization, isolation and soft-delete journey', async () => {
  ...
  expect((await publicApi.post('/api/auth/login', { data: { username: adminUsername, password: 'definitely-wrong' } })).status()).toBe(401)
  ...
  let humanId = ''
  let chatId = ''
  const username = `e2e_${runId.replace(/[^a-zA-Z0-9]/g, '').slice(-18).toLowerCase()}`
  ...
  try {
    ...
    const createdUser = await json(await admin.post('/api/admin/users', {
      data: { username, password: initialPassword, display_name: `远程验收 ${runId}`, role: 'user' },
    }), 201)
    testUserId = createdUser.id
    await json(await admin.patch(`/api/admin/users/${testUserId}`, { data: { display_name: `远程验收已更新 ${runId}` } }))
    ...
    await json(await user.post('/api/auth/change-password', { data: { current_password: initialPassword, new_password: changedPassword } }))
    await user.dispose()
    userLogin = await login(username, changedPassword)
    user = userLogin.api

    try {
      const createdProject = await json(await user.post('/api/projects', {
        data: { name: `REMOTE-E2E ${runId}`, artist: 'Codex', song_code: '10012204', description: 'remote contract test' },
      }), 201)
      projectId = createdProject.id
      await json(await user.patch(`/api/projects/${projectId}`, { data: { description: 'remote contract test updated' } }))
      expect((await json<any[]>(await user.get('/api/projects'))).some((item) => item.id === projectId)).toBeTruthy()
      expect((await admin.get(`/api/tasks/${projectId}`)).status()).toBe(404)

      const task = await json(await user.post(`/api/projects/${projectId}/tasks`, {
        data: { title: `Manual ${runId}`, storyboard_type: 'manual', overall_prompt: 'remote API acceptance' },
      }), 201)
      const taskId = task.id
      await json(await user.patch(`/api/tasks/${taskId}`, { data: { title: `Manual updated ${runId}` } }))
      await json(await user.get(`/api/tasks/${taskId}`))

      const styles = await json<any[]>(await user.get('/api/digital-human-styles'))
      expect(styles.length).toBeGreaterThan(0)
      const style = await json(await user.post('/api/digital-human-styles', { data: { name: `E2E style ${runId}` } }), 201)
      styleId = style.id
      ...
      humanId = human.id
      await json(await user.patch(`/api/digital-humans/${humanId}`, { data: { description: 'updated remote test character' } }))
      expect((await json<any[]>(await user.get('/api/digital-humans'))).some((item) => item.id === humanId)).toBeTruthy()
      await json(await user.put(`/api/tasks/${taskId}/cast`, { data: { digital_human_ids: [humanId] } }))

      const line1 = await json(await user.post(`/api/tasks/${taskId}/storyboard/lines`, {
        data: { source: 'manual', shot_type: 'character', planned_duration: 4, lyrics: 'remote line one', scene_prompt: 'scene one', shot_prompt: 'shot one', digital_human_ids: [humanId] },
      }), 201)
      const line2 = await json(await user.post(`/api/tasks/${taskId}/storyboard/lines`, {
        data: { source: 'manual', shot_type: 'empty', planned_duration: 15, lyrics: 'remote line two', scene_prompt: 'scene two', shot_prompt: 'shot two', digital_human_ids: [] },
      }), 201)
      await json(await user.patch(`/api/storyboard-lines/${line1.id}`, { data: { scene_prompt: 'scene one updated', digital_human_ids: [humanId] } }))
      await json(await user.post(`/api/tasks/${taskId}/storyboard/reorder`, { data: { line_ids: [line2.id, line1.id] } }))

      const general = await json(await user.post(`/api/projects/${projectId}/storyboards/general`, {
        data: {
          genre: 'pop', secondary_category: 'city', tertiary_category: 'night', season: 'autumn', age_group: 'adult', visual_style: 'cinematic',
          ratio: '9:16', resolution: '720p', image_model: 'gpt-image-2', video_model: 'doubao-seedance-2.0',
          empty_shot_count: 1, character_shot_count: 1, total_duration: 10, digital_human_ids: [humanId], extra_requirement: 'remote test',
        },
      }), 201)
      expect(general.lines).toHaveLength(2)
      await json(await user.get(`/api/tasks/${general.taskId}`))
      await json(await user.post(`/api/tasks/${general.taskId}/storyboard/retry-failed`, { data: {} }))

      const ass = await json(await user.post('/api/storyboards/ass', {
        multipart: {
          project_id: projectId, song_id: '10012204', digital_human_ids: JSON.stringify([humanId]), ratio: '16:9', resolution: '720p',
          image_model: 'gpt-image-2', video_model: 'doubao-seedance-2.0',
      ...
      if (realGeneration) {
        await json(await user.post(`/api/chat/${chatId}/messages`, { data: { text: 'reply with OK' } }), 202)
      } else {
        expect((await user.post(`/api/chat/${chatId}/messages`, { data: { text: '' } })).status()).toBe(422)
      }
    ...
    const createdError = errors.items.find((item: any) => item.path.includes(`remote-e2e-missing-${runId}`))
