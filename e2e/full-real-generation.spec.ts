...
test.skip(
  ...
  'requires real model, image, video and TOS providers',
)
...
const capture = async (page: Page, name: string) => {
  ...
  await page.screenshot({
    path: join(output, `${String(shot).padStart(2, '0')}-${name}.png`),
    fullPage: true,
  })
}
...
async function createProject(page: Page, name: string) {
  ...
  await expect(page.locator('.song-folder.current').filter({ hasText: name })).toBeVisible({
    timeout: 30_000,
  })
  await expect(
    page
...
async function generateAllMedia(page: Page, count: number, prefix: string) {
  ...
  await expect(page.locator('.line-wrapper .shot-thumb img')).toHaveCount(count, {
    timeout: 15 * 60_000,
  })
  ...
  await expect(page.getByRole('button', { name: '素材已导出' })).toBeVisible({
    timeout: 5 * 60_000,
  })
