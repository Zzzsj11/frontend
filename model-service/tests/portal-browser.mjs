...
try {
 ...
 const routeSwitch=admin.getByRole('switch',{name:'quote-browser-route启用',exact:true})
 await expect(routeSwitch).toBeDisabled()
 await expect(routeSwitch).toHaveAttribute('aria-checked','false')
 await expect(admin.getByRole('columnheader',{name:'启用',exact:true})).toHaveCSS('white-space','nowrap')
 await admin.screenshot({path:path.join(root,'.runtime','routes-desktop.png'),fullPage:true})
 await admin.setViewportSize({width:390,height:844})
 if(await admin.evaluate(()=>document.documentElement.scrollWidth>window.innerWidth))throw Error('Routes panel overflows mobile viewport')
 await admin.setViewportSize({width:1440,height:1100})
 await admin.getByRole('button',{name:'任务与用量',exact:true}).click()
...
asyncio.run(run())`],{PYTHONPATH:path.join(root,'public-api')})
 ...
 const searchBounds=await user.getByRole('search',{name:'模型搜索'}).boundingBox()
 const catalogBounds=await user.getByRole('region',{name:'API 模型目录'}).boundingBox()
 if(Math.abs(searchBounds.x+searchBounds.width/2-catalogBounds.x-catalogBounds.width/2)>2)throw Error('Model search is not centered')
 await expect(user.getByRole('search',{name:'模型搜索'}).getByRole('button')).toHaveCount(3)
 await user.screenshot({path:path.join(root,'.runtime/portal-docs.png'),fullPage:true})
 await expect(user.getByText('一次接入，多种模型',{exact:true})).toHaveCount(0)
 await expect(user.getByText('API REFERENCE',{exact:true})).toHaveCount(0)
 await expect(user.getByRole('button',{name:'删除 浏览器验收 Key',exact:true})).toHaveCount(0)
 await user.getByRole('button',{name:'撤销 浏览器验收 Key',exact:true}).click()
 await expect(user.getByRole('dialog',{name:'撤销 API Key'})).toBeVisible()
 await user.getByRole('button',{name:'取消',exact:true}).click()
 await expect(user.getByRole('button',{name:'撤销 浏览器验收 Key',exact:true})).toBeVisible()
 await user.getByRole('button',{name:'撤销 浏览器验收 Key',exact:true}).click()
 await user.getByRole('button',{name:'确认撤销',exact:true}).click()
 await expect(user.getByText('已撤销',{exact:true})).toBeVisible()
 await expect(user.getByRole('button',{name:'撤销 浏览器验收 Key',exact:true})).toHaveCount(0)
 if((await fetch('http://127.0.0.1:8191/v1/models',{headers:{Authorization:'Bearer '+revealedKey.trim()}})).status!==401)throw Error('Revoked key still authenticates')
 await user.getByRole('region',{name:'我的 API Key'}).screenshot({path:path.join(root,'.runtime/key-revoked.png')})
 await user.getByRole('button',{name:'删除 浏览器验收 Key',exact:true}).click()
 await expect(user.getByRole('dialog',{name:'删除 API Key'})).toBeVisible()
 await user.getByRole('button',{name:'确认删除',exact:true}).click()
 await expect(user.getByRole('button',{name:'删除 浏览器验收 Key',exact:true})).toHaveCount(0)
 await user.getByRole('button',{name:'积分账户',exact:true}).click()
 await user.getByRole('button',{name:'刷新积分',exact:true}).click()
 await expect(user.getByRole('button',{name:'fixture-browser-job',exact:true})).toBeVisible()
 await user.getByRole('button',{name:'API',exact:true}).click()
 await user.reload()
