...
try {
 ...
 await user.getByRole('button',{name:'登录 / 注册'}).click()
 await user.getByRole('button',{name:'创建账号'}).click()
 await expect(user.getByText('@star-net.cn',{exact:true})).toBeVisible()
 await user.getByLabel('公司邮箱前缀',{exact:true}).fill('intruder@example.com')
 if(await user.getByLabel('公司邮箱前缀',{exact:true}).evaluate(el=>el.checkValidity()))throw Error('Foreign domain accepted by registration field')
 await user.getByLabel('公司邮箱前缀',{exact:true}).fill('portal_acceptance')
