...
try {
 ...
 await expect(user.getByRole('button',{name:'我的用量与积分'})).toHaveCount(0)
 ...
 await user.getByRole('button',{name:'登录',exact:true}).click()
 await expect(user.getByText('暂无 Key，请创建自己的 API Key。')).toBeVisible()
 ...
 await expect(user.getByText('Key 仅展示一次，请妥善保存，不要放入前端代码。')).toBeVisible()
...
`],{PYTHONPATH:path.join(root,'public-api')})
 ...
 await expect(user.getByText(/本月已用 20.000000/).first()).toBeVisible()
 await expect(user.getByText(/可用 980.000000/).first()).toBeVisible()
 await expect(user.getByRole('cell').filter({hasText:'按用量计价'})).toBeVisible()
 await expect(user.getByText('月上限调整',{exact:true})).toBeVisible()
