...
- 账号统一由管理员在「用户与积分」创建，仅支持 `@star-net.cn` 公司邮箱；公开注册接口返回 403。前端固定邮箱后缀，后端拒绝其他域名、子域名和伪造后缀；不提供邮箱归属验证。历史账号保持登录兼容，不自动删除或改名。用户名不区分大小写，初始及新密码至少 8 位，必须同时包含字母和数字，采用随机盐 scrypt；登录令牌是随机、不透明、数据库仅存散列的 8 小时会话。退出软删除会话，立即失效。浏览器只在内存保存令牌，刷新后重新登录。
- 迁移 `0007` 为现有与新用户启用首次改密：首次登录只允许查看身份、改密、退出，改密前不能访问用量、积分或使用绑定 Key。改密需验证原密码，新密码不能相同，成功后撤销全部旧会话并重新登录。既有系统 Key 不受影响。
- 登录页可勾选「记住用户名」，仅在本机保存邮箱前缀，不保存密码或令牌，取消勾选立即清除。
- 登录及改密按来源 IP 记录尝试并限速；生产反代必须正确配置可信代理范围，不能信任任意客户端 X-Forwarded-For。
- 用户登录且完成首次改密后，自行创建 Key、设置各 Key 月消费上限及删除自己的 Key。每账号最多 10 个未删除 Key（停用仍占名额）；服务端在事务锁内校验。归属从登录会话确定，不能指定他人账号。Key 明文只在创建时返回一次，后续仅展示前缀；不写审计或浏览器持久存储。
- 管理员只开通账号和设置账号月度总额度；初始密码可自动生成，仅展示一次。用户不能修改账号总额度，管理员不再通过绑定接口创建或转移用户 Key；系统 Key 保持原管理方式。
- 迁移 `0008` 增加账号总上限，现有账号按未删除 Key 的配置额度合计初始化，无 Key 则为 0，保留现有密码和消费。账号和 Key 上限可立即调整，保留本月消费与预占，不通过调整额度重新发放已用积分。
...
账号总上限由管理员维护；用户自行分配各 Key 上限，其合计可以超过账号总额。接单同时校验 Key 钱包、Key 月上限减当月消费与预占、账号总上限减全部 Key 的当月消费与预占。多 Key 并发共用数据库事务锁，不能各自重复花掉同一份账号额度。停用或软删除 Key 不移除已用及在途预占。后台和用户端可用额度显示不低于零。

月消费按北京时间自然月的结算流水（task_charge / reconciliation）统计，月初自然更新，不累计到下月。跨月在途预占保留，结算计入结算所在月；退款/核账差额计入入账月。Key 月上限调整立即生效，账本记录差额，既有消费和预占保留。账号总额下调至已消费以下或 Key 上限低于预占时，已有任务继续结算，新请求拒绝。

上限是提交前的额度控制：若真实供应商费用超过预占，仍如实记账并阻止后续提交，不截断账单或将超额成本记为零。因此预占应覆盖单次请求的最大合理费用。
...
- `GET /portal/ledger?page=1&limit=50&client_id=...&kind=...`：自己的积分流水，按 Key 和类型筛选
- `GET /portal/history-keys`：当前用户的 Key 名称列表（包含已软删除 Key），供历史筛选使用
- `GET /portal/jobs/{id}`：自己的任务详情，跨用户访问返回 404

任务与流水分页返回 `items / total / page / limit`，每页 1–100 条；超出末页时返回末页。历史查询按用户及 Key 所有权双重隔离，Key 软删除后仍保留该用户历史。

管理员（独立管理 Token）：

- `GET /admin/users`：账号、月总上限、本月消费、预占、可用、Key 数量及分配合计
- `POST /admin/users`：企业邮箱、monthly_points、可选 initial_password；返回一次初始密码
- `POST /admin/users/{id}/quota`：设置账号月总上限（points），立即生效
- `POST /admin/clients`：仅创建系统 Key，携带 user_id 返回 403
- `POST /admin/clients/{id}/bind`：已关闭，返回 403
- `GET /admin/wallets`：各 Key 钱包
- `POST /admin/clients/{id}/quota`：`{"points":"10000"}`，仅系统 Key 设置下月额度
...
`make check` 覆盖管理员建号、自助 Key、10 Key 并发限制、账号总额、跨月、所有权、权限、费率模式、缓存 Token 去重计价、价格快照、并发额度、幂等结算、月度重置和临时增减。`node tests/portal-browser.mjs` 使用隔离临时数据库及 8191–8194 端口验收全链路；没有供应商密钥，也不启动 Worker 或发模型请求。`tests/postgres-concurrency.py` 另在隔离 PostgreSQL 验证跨事务并发。
...
管理后台「修改密码」验证原密码，新密码至少 8 位且包含字母和数字，确认输入必须一致且不同于原密码。`POST /admin/change-password` 成功后递增数据库中的认证版本，所有旧管理令牌立即失效，返回登录页。

迁移 `0009` 增加含生命周期字段的 `admin_credentials`。现有环境变量中的初始管理员密码只在首次成功登录、尚无该账号记录时导入 scrypt 哈希；之后以数据库为准，重启及更改环境变量不会覆盖已保存密码，软删除记录也不会触发初始密码回退。密码不进入日志、审计或浏览器持久存储。登录失败与改密失败都有 10 分钟窗口限速。生产 `model_public` 角色无此表权限；迁移后执行 bootstrap 的 grant 步骤再发布管理端。此次发布使旧格式管理令牌失效，需要重新登录一次。

## 用户端积分账户与 API 页面

用户端分为「积分账户」与「API」。积分账户展示账号本月可用、月度总额度、已消费和预占，标明北京时间月初更新；无充值、支付或兑换入口。积分统一显示整数并带千位分隔，金额四舍五入显示两位小数；账本存储与计费运算精度不变。流水支持 Key、变动类型筛选、页码跳转及每页条数，关联任务可打开详情。流水余额明确为对应 Key 的记账余额，不作为账号剩余额度。

API 页面使用紧凑 Key 列表，创建、修改月上限与删除采用弹窗；Key 明文只在创建后显示一次，支持复制。模型文档按文本、图像、视频及公司平铺，支持搜索、复制命令与当前模型文档。未开放模型标记状态，专用工作流不提供通用接口示例。示例使用环境变量占位，不写入用户真实 Key，API 基础地址由管理员提供。桌面与手机布局均纳入隔离浏览器验收。
