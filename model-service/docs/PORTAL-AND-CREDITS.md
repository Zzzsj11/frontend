...
- 用户仅可使用 `@star-net.cn` 公司邮箱注册。前端固定邮箱后缀，后端拒绝其他域名、子域名和伪造后缀；不提供邮箱归属验证。历史账号保持登录兼容，不自动删除或改名。用户名不区分大小写，密码至少 10 位，采用随机盐 scrypt；登录令牌是随机、不透明、数据库仅存散列的 8 小时会话。退出软删除会话，立即失效。浏览器只在内存保存令牌，刷新后重新登录。
