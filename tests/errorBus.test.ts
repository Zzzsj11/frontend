...
describe('global error dialog queue', () => {
  ...
  it('translates browser-owned network errors but preserves upstream API messages', () => {
    expect(reportApiError(new TypeError('Failed to fetch'), '网络连接失败').message).toBe('网络连接失败')
    expect(reportApiError(new ApiError('Request timed out.', 502)).message).toBe('Request timed out.')
  })
})
