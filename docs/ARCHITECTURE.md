...
Chat对话、媒体生成与素材导出支持两种执行模式：默认 `inline` 保持原单进程行为；服务器设置 `JOB_EXECUTION_MODE=worker` 后，API 只把工单持久化到 `generation_jobs`，`worker-chat`、`worker-media` 与 `worker-export` 使用 PostgreSQL `FOR UPDATE SKIP LOCKED` 领取。Redis只承担低延迟唤醒、Chat跨进程取消、热状态与事件，短暂不可用不会丢失数据库工单。单机Worker进程内继续按模型注册中心的 `executionPool/executionConcurrency` 限流，H3上限为2；扩到多机/K8s前仍需把该限制升级为Redis原子租约。ASS大纲和场景段重试目前仍在API后台协程，需结合领域状态机单独迁移。新模型接入现状和剩余清单见 `TODO_MODEL_EXPANSION.md`。
