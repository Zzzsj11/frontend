...
人物身份参考卡生成（2026-09-21）：默认使用英和 `gpt-image-2.5-sunburst`，可通过
`DIGITAL_HUMAN_IMAGE_MODEL` 覆盖；显式指定的模型保持优先。系统人物 001 的版式参考使用
`originalAvatar` 公网原图，禁止把列表缩略图用于版式参考。上传与重新生成仅把用户填写的
身份特征放入提示词，空描述不得回退到人物名称（如“女10”）；资产分类 `style` 只用于归档，
不再拼接成画面风格。模型默认值在创建工单前写入请求快照，Worker 与用量记录使用同一模型。
