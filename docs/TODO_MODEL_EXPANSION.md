...
- FLUX/BFL 创建任务后保存供应商 `polling_url`，正常轮询、重启恢复和管理端同步均使用同一地址；只允许配置的服务域名或 BFL HTTPS 域名，并核对任务 ID。旧工单没有保存此字段，仍需从供应商恢复原始创建响应，不能把查询 404 当作未扣费的依据。
- 管理端同步识别 BFL `Ready`、Kling `task_status=succeed` 等成功状态；显式对账恢复允许已失败工单补回资产和成功状态，并重新对账。普通 worker 的超时终态保护保持生效。
- Gemini 保持文档规定的顶层 `prompt`、`images`；单张人物身份图使用 `reference_to_video`，普通单张场景图使用 `image_to_video`。供应商“缺少图片”仍需真实补测确认，不能仅凭字段契约测试宣称解决。
...
- 上述三种适配器保存脱敏的实际请求体到工单，管理员可通过视频对账详情的 `providerRequest` 核查；不记录鉴权头。
- 补测仅覆盖原失败、未执行和 Kling 人物不合格镜头；已有合格结果不重生成。全部维持 720 档、Grok/Gemini 10 秒，HappyHorse/Vidu/Veo 继续排除。所有真实调用必须携带 Agent 与测试批次标识。
