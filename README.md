...
- 管理控制台：仪表盘、项目/用户/生成任务/费用用量、提示词管理、通用分类全量自定义（树形 CRUD + 种子迁移）、模型注册中心启停、Gemini/MiniMax 多参考提示词优化、Kling 与 RunningHub H3 工作流测试面板、错误日志/操作审计/LLM 调用/接口耗时/性能页；侧边导航按「总览 / 业务运营 / 内容配置 / 模型实验室 / 系统监控」五组两级分组。
