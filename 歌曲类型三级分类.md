...
**已接入**通用分镜弹框（通用 MV 视频），替换掉 mock 时代的占位选项（流行/摇滚/民谣）。分类树定义在 `src/songCategories.ts`（`SONG_CATEGORY_GENRES`),value 与 label 均为中文分类名 —— 表单提交时经 `labelOf` 直接把中文分类名写入 `storyboard_config`，与曲库打标数据口径完全一致，也使 storyBible logline 与 LLM 上下文保持中文语义。
