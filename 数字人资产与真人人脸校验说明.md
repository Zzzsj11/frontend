...
1. **创建**：`POST {VIDEO_API_BASE_URL}/virtual/assets/create`
   ```json
   {"url": "<图片公开URL>", "name": "mv-001", "assetType": "Image", "Moderation": {"Strategy": "Skip"}}
   ```
...
2. **启动时后台补扫**：`seed.py::ensure_pending_asset_avatars()`，lifespan 里 `asyncio.create_task`，处理所有"活跃且 asset 为空"的数字人（系统 + 用户上传）
3. **cron 每分钟补扫**：独立脚本 `backend/scripts/ensure_asset_avatars.py`，每分钟扫一次补齐，保证上游抖动导致的失败**最多 1 分钟内**被修复
...
| `backend/app/main.py` | `_resolve_asset_avatar_urls()` 视频生成 URL 映射；lifespan 启动兜底任务 |
