...
const outlinePercent = computed(() => {
  ...
  // 时间爬升：按约 150 秒估算，封顶 95%，无进度事件期间也保持缓慢递增
  const timeBased = Math.min(95, (outlineElapsed.value / 1000 / 150) * 100)
  let stageBased = 5
  if (progress?.phase === 'segments' && progress.segmentsTotal)
    stageBased = 15 + 75 * ((progress.segmentsDone ?? 0) / progress.segmentsTotal)
  else if (progress?.phase === 'planning') stageBased = 10
    ...
    >
      <template v-if="store.outlinePhase === 'outlining'">
        ...
        <span class="banner-text">
          正在生成 MV 大纲（{{ outlineStageText }}），本轮约需 2 分钟，请稍候 · 已等待
      ...
      <template v-else>歌词时间轴拆分完成，准备生成大纲</template>
    </div>
