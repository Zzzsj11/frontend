...
const regenerate = async () => {
  if (!await confirmDialog({
    title: '重新生成分镜大纲',
    message: '重新生成后，每镜的人物/空镜规划和人物分配都会更新，现有分镜提示词将重新生成。是否继续？',
    confirmText: '重新生成',
  })) return
...
<template>
  <div v-if="store.outlineOpen && store.activeStoryBible" class="outline-mask" @click.self="store.closeOutline()">
    <section class="outline-modal" role="dialog" aria-modal="true" aria-label="分镜大纲总览">
      <header>
        <div><h3><AppIcon name="file" :size="17" /> 分镜大纲总览</h3><p>{{ store.activeStoryBible.logline }}</p></div>
      ...
      <div class="outline-list">
        <article v-for="shot in store.activeStoryBible.shots" :key="shot.index" class="outline-shot">
          ...
          <div class="shot-content">
            ...
            <p v-if="shot.lyrics" class="lyrics">{{ shot.lyrics }}</p>
      ...
      <footer>
        <span>该大纲将指导后续每条分镜提示词生成，只支持查看。</span>
