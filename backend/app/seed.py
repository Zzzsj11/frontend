...
from .models import AdminPermissionModel, AdminRoleModel, AdminRolePermissionModel, AiModelModel, AiProviderModel, DigitalHumanModel, DigitalHumanStyleModel, SongEmotionProfileModel, StoryboardLineModel, UserAdminRoleModel, UserModel, utcnow
...
async def seed_system_data() -> None:
    async with session_factory() as session:
        ...
        if not role:
            ...
            session.add(role)
        for code, name in [("dashboard.read","查看仪表盘"),("users.manage","管理用户"),("models.manage","管理模型"),("assets.manage","管理系统资产"),("logs.read","查看日志")]:
            pid = f"admin-perm-{code.replace('.', '-')}"
            permission = await session.get(AdminPermissionModel, pid)
            if not permission:
                permission = AdminPermissionModel(id=pid, code=code, name=name); session.add(permission)
        ...
        if not provider:
            provider = AiProviderModel(id="provider-yinghe", code="yinghe", name="银河 API", base_url="", status="active"); session.add(provider)
        defaults = [
        ...
        for mid, code, name, modality, provider_id, capabilities in defaults:
            ...
            if not model:
                session.add(AiModelModel(id=mid, provider_id=provider.id, code=code, name=name, modality=modality, provider_model_id=provider_id or code, capabilities=capabilities, status="active", user_visible=True, is_default=True))
