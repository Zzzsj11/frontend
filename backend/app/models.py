...
class DigitalHumanModel(LifecycleMixin, Base):
    ...
    # PPIO 账号下的独立虚拟人物资产；供应商资产 ID 不可跨账号/渠道复用。
    ppio_asset_avatar_url: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class StoryboardLineModel(LifecycleMixin, Base):
    ...
    reference_media: Mapped[list[dict[str, Any]]] = mapped_column(JSON, default=list)
    optimizer_provider: Mapped[str] = mapped_column(String(32), default="")
    optimization_task_id: Mapped[str | None] = mapped_column(ForeignKey("prompt_optimization_tasks.id", ondelete="SET NULL"), nullable=True, index=True)
...
class GenerationJobModel(LifecycleMixin, Base):
    ...
    generation_origin: Mapped[str] = mapped_column(String(32), default="business", server_default="business", index=True)
    agent_name: Mapped[str] = mapped_column(String(80), default="", server_default="", index=True)
    agent_run_id: Mapped[str] = mapped_column(String(160), default="", server_default="", index=True)
...
class ShotAssetModel(LifecycleMixin, Base):
    ...
    requested_resolution: Mapped[str] = mapped_column(String(32), default="1080p")
    provider_resolution: Mapped[str] = mapped_column(String(32), default="")
    actual_width: Mapped[int | None] = mapped_column(Integer, nullable=True)
    actual_height: Mapped[int | None] = mapped_column(Integer, nullable=True)
...
class ServerMetricSampleModel(LifecycleMixin, Base):
    ...
    cpu_iowait_percent: Mapped[float] = mapped_column(Float, default=0)
    ...
    swap_free_bytes: Mapped[int] = mapped_column(BigInteger, default=0)
    ...
    disk_read_bps: Mapped[float] = mapped_column(Float, default=0)
    disk_write_bps: Mapped[float] = mapped_column(Float, default=0)
    disk_read_iops: Mapped[float] = mapped_column(Float, default=0)
    disk_write_iops: Mapped[float] = mapped_column(Float, default=0)
    filesystems: Mapped[list[dict[str, Any]]] = mapped_column(JSON, default=list)
    containers: Mapped[list[dict[str, Any]]] = mapped_column(JSON, default=list)
    workloads: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)
